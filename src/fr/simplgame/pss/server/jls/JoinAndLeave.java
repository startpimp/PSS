...
public class JoinAndLeave {
	...
	public static void roleAdd(MessageChannel channel, Message message, Loader[] loader, Member member) {
		...
		String value = message.getMentionedRoles().get(0).getId();
		String msg = message.getMentionedRoles().get(0).getAsMention();
		
		System.out.println(message.getMentionedRoles().size());

		for (int i = 1; i < message.getMentionedRoles().size(); i++) {
			value += "_" + message.getMentionedRoles().get(i).getId();
			msg += loader[0].lang.get("general.mark.comma") + message.getMentionedRoles().get(i).getAsMention();
		}

		CSV.modifyCell(message.getGuild().getId(), "join_role", value, "./res/server.csv");

		channel.sendMessage(loader[0].lang.get("command.jls.jal.L2").replace("[ROLE]", msg)).queue();
